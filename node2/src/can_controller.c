 ...
 */
uint8_t can_init_default_tx_rx_mb(uint32_t can_br) { return can_init(can_br, 1, 2); }
...
uint8_t can_init(uint32_t can_br, uint8_t num_tx_mb, uint8_t num_rx_mb) {
    ...
    (void)CAN0->CAN_SR; // Clear status register on read
    ...
    PMC->PMC_PCR =
      PMC_PCR_EN | (0 << PMC_PCR_DIV_Pos) | PMC_PCR_CMD | (ID_CAN0 << PMC_PCR_PID_Pos); // DIV = 1(can clk = MCK/2), CMD = 1 (write), PID = 2B (CAN0)
